...
export default class OfferMaker extends React.Component {
  ...
  render() {
    ...
    if (this.props.side === 'buy') {
      title = `  用${counterAssetName} 买成${baseAssetName}`;
    } else {
      title = `用${counterAssetName}卖成 ${baseAssetName}`;
    }
    ...
    if (this.props.d.session.state === 'in') {
      ...
      if (targetBalance) {
        ...
        youHave = <div>
          <p className="OfferMaker__youHave">必须 <a href="#account/addTrust">接受资产 {targetAsset.getCode()}</a> 再交易</p>
    ...
    if (this.state.valid) {
      if (this.props.side === 'buy') {
        summary = <div className="s-alert s-alert--info">买 {this.state.amount} {this.capDigits(baseAssetName)} 为 {this.capDigits(this.state.total)} {counterAssetName}</div>;
      } else {
        summary = <div className="s-alert s-alert--info">卖 {this.state.amount} {this.capDigits(baseAssetName)} 为 {this.capDigits(this.state.total)} {counterAssetName}</div>;
      }
    }
      ...
      <form onSubmit={this.handleSubmit}  disabled={!this.state.valid || this.state.buttonState === 'pending'}>
        <table className="OfferMaker__table">
          <tbody>
            ...
            <tr className="OfferMaker__table__row">
              <td className="OfferMaker__table__label">总量</td>
              <td className="OfferMaker__table__input">
                <label className="OfferMaker__table__input__group">
